...
class FPSEnv(gym.Env):
    ...
    def add_obj(self, name, is_enemy, pos, leader_objid, team_id, dir=[0, 0, 0], model_name='DefaultAI', weapon='m4'):
        ...
        # if s.find('success')==-1:
        #   self.reset()
        time.sleep(0.1)#针对vr版本，避免Dictionary线程不安全的问题
        return  # s
    ...
    def attack(self, objid_list, auth='normal', pos='replace', walkType='run', reachDist=12):
        '''
        ...
        cmd += '<check name="CheckTimeChk" interval="0"><action name="ShootAct"/><action name="MoveToPosAct" destObj="target" walkType="%s"  reachDist="%d"/></check>'%(walkType, reachDist)
        # print(cmd)
        self.client.send(cmd)
        # s = self.client.receive()
        self.cmdThreadsLen -= 1
