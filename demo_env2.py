...
class DemoEnv2(fps.FPSEnv):
    ...
    def __init__(self,):
        ...
        self.war_despos=[-225.5,-0.49,23.1]
        self.war_pos=[-226.1,-1,27.1]
        self.comradeteam=[]
        self.enemyteam=[]
    ...
    def set_enemynearby(self,objid_list,dist=30):
        enemylist=[]
        for i in objid_list:
            curenemy=[]
            for enemyid in self.comradeteam:
                #print(self.curenemyfeature)
                curidpos=self.curenemyfeature[i]['POSITION']
                curidposxy=np.array([curidpos[0],curidpos[2]])
                pos=self.curcomradefeature[enemyid]['POSITION']
                posxy=np.array([pos[0],pos[2]])
                gap=curidposxy-posxy
                dis=np.sqrt(np.vdot(gap,gap))
                if dis<=dist:
                    curenemy.append(enemyid)
            enemylist.append(curenemy.copy())
        return enemylist

    def enemyai(self):
        enemy=self.set_enemynearby(self.enemyteam)
        for i in range(len(self.enemyteam)):
            if len(enemy[i])>=1:
                #print('bbbbbbbb')
                selectid=random.sample(enemy[i],1)
                self.set_target_objid(objid_list=[self.enemyteam[i]],targetObjID=selectid[0])
                self.attack(objid_list=[self.enemyteam[i]],auth='top')
    ...
    def get_feature(self):
        ...
        if len(list(vardict.keys()))<11:
            pirnt('we get right info')
        featuredict={}
        enemydict={}
        comradedict={}
        gameover=False
